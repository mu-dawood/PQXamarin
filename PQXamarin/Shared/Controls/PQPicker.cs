...
namespace PQXamarin.Controls
{
    public class PQPicker : Picker
    {
        ...
        public PQPicker()
        {
            this.SelectedIndexChanged += PQPicker_SelectedIndexChanged; ;
            Focused += PQEntry_Focused;
            Unfocused += PQEntry_Unfocused;
        }

        private void PQPicker_SelectedIndexChanged(object sender, EventArgs e)
        {
            Validate();
        }
            ...
            if (propertyInfo != null && this.BindingContext != null)
            {
                ...
                if (_attributes != null && _attributes.Count() > 0)
                {
                    foreach (var attr in _attributes)
                    {
                        ...
                        if (!IAttr.Validate(BindingContext, SelectedItem))
                        {
                            HasError = true;
                            if (ShowError)
                            {
